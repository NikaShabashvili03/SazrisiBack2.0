...
class AttemptAISummaryView(APIView):
    """
    GET  /attempts/<attempt_id>/ai-summary/  — returns the latest AI summary for this attempt.
    POST /attempts/<attempt_id>/ai-summary/  — generates and stores a new AI study plan.
    """
    permission_classes = [IsAuthenticated]

    def _get_attempt(self, request, attempt_id):
        try:
            return QuizAttempt.objects.get(id=attempt_id, user=request.user), None
    ...
    def get(self, request, attempt_id):
        attempt, err = self._get_attempt(request, attempt_id)
        if err is not None:
            return err

        summary = (
            QuizAISummary.objects.filter(user=request.user, attempt=attempt)
            .order_by('-created_at')
            .first()
        )
        if summary is None:
        ...
        return Response(QuizAISummarySerializer(summary).data, status=status.HTTP_200_OK)
